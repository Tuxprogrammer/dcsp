...
//            $finfo = mime_content_type($_FILES['upfile']['tmp_name']);
        ...
        if (!move_uploaded_file($_FILES['upfile']['tmp_name'], sprintf('./media/%s.%s', $name, $ext))) {
        ...
        if (isset($row['avatarImage']) && !empty($row['avatarImage']) && realpath($row['avatarImage']) && is_writable($row['avatarImage'])) {
